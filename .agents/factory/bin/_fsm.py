"""Shared helpers for the RCAC-Docs documentation-factory FSM scripts.

The finite-state machine for a documentation job (a `feature`/`fix`/`refactor`)
lives in the YAML frontmatter of its ``spec/<slug>/TECH.md`` roadmap. These
helpers read, validate, mutate, and re-serialize that frontmatter so the
*scripts* (not the model) own the fragile YAML arithmetic — model in-context
YAML editing is the primary FSM-corruption risk (see
