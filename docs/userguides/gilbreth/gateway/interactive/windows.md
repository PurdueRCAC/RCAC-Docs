...
The Windows Desktop app will launch a Windows desktop session on a compute node. This is similar to using the Windows menu launcher through [Thinlinc](/knowledge/${resource.hostname}/accounts/login/thinlinc), however, this gives you a Windows desktop directly on a compute node instead on a front-end.
