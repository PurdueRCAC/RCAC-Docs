...
* Use `slist` to determine which queues (`--account` or `-A` option) are available to you. The name of the queue which is available to everyone on Gilbreth is "${resource.queue}".
