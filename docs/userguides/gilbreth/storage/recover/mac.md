...
To use the automated command-line tool, log into a compute cluster or into the host `${resource.hostname}.rcac.purdue.edu` (which is available to all Gilbreth users) and use the [flost](/knowledge/${resource.dir}/recover/flost) tool. On Mac OS X you can use the built-in SSH terminal application to connect.
...
To use the automated command-line tool, log into a compute cluster or into the host `${resource.hostname}.rcac.purdue.edu` (which is available to all Gilbreth users) and use the [flost](/knowledge/${resource.dir}/storage/recover/flost) tool. On Mac OS X you can use the built-in SSH terminal application to connect.
