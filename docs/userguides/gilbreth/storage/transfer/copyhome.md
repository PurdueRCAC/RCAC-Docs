...
Please note that copying may fail if the size of your research computing home directory is larger than the Gilbreth one's quota. Please [check](/knowledge/${resource.hostname}/storage/quota) usage and limits before proceeding!
...
* **Example:** copying using Globus.
  ...
  Search collections for *"Purdue Research Computing - Home Directories"* and *{::if resource.unifiedendpoint== true} "Purdue Gilbreth Cluster" {::else} "Purdue Gilbreth Cluster - Home" {::/}* endpoints, respectively, then transfer desired files and/or directories as usual.