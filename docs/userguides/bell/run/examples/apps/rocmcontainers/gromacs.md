...
Submit a Slurm job, making sure to request GPU-enabled queue and desired number of GPUs. The following example shows an Slurm job submission, asking for one node (${resource.nodecores} cores) in the "gpu" account with and two GPUs for 6 hours:
