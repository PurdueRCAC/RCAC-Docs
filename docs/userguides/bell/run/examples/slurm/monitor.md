...
Knowing the precise resource utilization an application had during a job, {::if resource.qsub\_needs\_gpu == 1} such as GPU load or memory, {::else} such as CPU load or memory, {::/} can be incredibly useful. This is especially the case when the application isn't performing as expected.
...
Complete documentation is available online at [resource-monitor.readthedocs.io](https://resource-monitor.readthedocs.io). A full manual page is also available for reference, `man monitor`.
...
{::if resource.name == Negishi or if resource.name == Gautschi or if resource.name == Gilbreth} module load monitor {::else} module load monitor {::/}
