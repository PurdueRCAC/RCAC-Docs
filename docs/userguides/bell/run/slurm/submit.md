...
By default, jobs on ${resource.name} will share nodes with other jobs.
...
​On ${resource.name}, in order to submit jobs, you need to specify the partition, account and Quality of Service (QoS) name to which you want to submit your jobs. To familiarize yourself with the partitions and QoS available on ${resource.name}, visit [Negishi Queues and Partitions](https://www.rcac.purdue.edu/knowledge/negishi/run/slurm/queues). To check the available partitions on ${resource.name}, you can use the `showpartitions` , and to check your available accounts you can use `slist` commands. Slurm uses the term "Account" with the option `-A` or `--account=` to specify different batch accounts, the option `-p` or `--partition=` to select a specific partition for job submission, and the option `-q` or `--qos=` .
...
The CPU partition on ${resource.name} has two Quality of Service (QoS) levels: **normal** and **standby**. To submit your job to one compute node on `cpu` partition and 'normal' QoS which has "high priority":
...
On the GPU partition on **${resource.name}** you don’t need to specify the QoS name because only one QoS exists for this partition, and the default is **normal**. To submit your job to one compute node requesting one GPU on the `gpu` partition under the 'normal' QoS which has "high priority":
...
​On ${resource.name}, in order to submit jobs, you need to specify the partition, account and Quality of Service (QoS) name to which you want to submit your jobs. To familiarize yourself with the partitions and QoS available on ${resource.name}, visit [Bell Queues and Partitions](https://www.rcac.purdue.edu/knowledge/bell/run/slurm/queues). To check the available partitions on ${resource.name}, you can use the `showpartitions` , and to check your available accounts you can use `slist` commands. Slurm uses the term "Account" with the option `-A` or `--account=` to specify different batch accounts, the option `-p` or `--partition=` to select a specific partition for job submission, and the option `-q` or `--qos=` .
...
{::if resource.name == Gilbreth}Each compute node in ${resource.name} has various cores per node. Refer to the [Hardware Overview](/knowledge/${resource.dir}/overview) and [Queue Overview](/knowledge/${resource.dir}/run/slurm/queues) for details.{::else}Each compute node in ${resource.name} has ${resource.nodecores} processor cores.{::/}
