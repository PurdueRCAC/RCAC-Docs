...
You can transfer files both to and from ${resource.name} while initiating an SFTP session on either some other computer or on ${resource.name} (in other words, directionality of connection and directionality of data flow are independent from each other). Once the connection is established, you use `put` or `get` subcommands between "local" and "remote" computers. Either ${resource.name} or another computer can be a remote.

* **Example:** Initiating SFTP session on some other computer (i.e. you are on another computer, connecting to ${resource.name}):

  ```
  $ sftp ${user.username}@${resource.hostname}.rcac.purdue.edu

        (transfer TO ${resource.name})
  sftp> put sourcefile somedir/destinationfile
  sftp> put -P sourcefile somedir/

        (transfer FROM ${resource.name})
...
* **Example:** Initiating SFTP session on ${resource.name} (i.e. you are on ${resource.name}, connecting to some other computer):
