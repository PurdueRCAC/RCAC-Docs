...
## Example
Here is an example using an Ubuntu 16.04 image on {CLUSTER}:
...
Everything you need on how to [build a container](https://apptainer.org/docs/user/main/build_a_container.html) is available from their user guide. Below are merely some quick tips for getting your own containers built for {CLUSTER}.
...
Finally, copy the new image to {CLUSTER} and run it.