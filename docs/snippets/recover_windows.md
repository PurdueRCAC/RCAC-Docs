<!-- Template snippet: {CLUSTER} = title-case cluster name, {cluster} = lowercase. Replaced at build time by `recover_windows_snippet` in main.py. -->
# Windows

If you use {CLUSTER} through "network drives" on Windows you may recover lost files directly from within Windows:
