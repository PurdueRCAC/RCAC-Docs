...
# Gilbreth Agent Context

This file is the assembled agent context for Purdue RCAC's **Gilbreth**
cluster, concatenated from the topic files in `/etc/agents.d/`. It is the
canonical guidance an agent should follow when operating on Gilbreth. Read it
before acting.
...
Connecting to `gilbreth.rcac.purdue.edu` places you on one of several shared front-end
...
# Filesystems (Gilbreth)

Gilbreth mounts several filesystems, each with a different purpose, technology, and
...
## Scratch — `/scratch/gilbreth/$USER` (`$RCAC_SCRATCH`)
...
- Use of Gilbreth is bound by Purdue IT's Acceptable Use Policy and RCAC's resource
...
- Gilbreth uses **Apptainer** (not Docker). RCAC's Apptainer configuration
